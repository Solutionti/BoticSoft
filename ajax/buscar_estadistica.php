...
if($q4==">=1"){
 ...
 for($i=$f1;$i<=$f2;$i++){
     ...
     if($q==3){
     if($i<$f2){
         $text=$text." SUM(IF(DATE_FORMAT(fecha, '%Y-%m-%d')='$fec[$j]' and tipo_doc<=3 and ven_com=$q1 and activo=1 and detalle_factura.tienda$tienda,cantidad*precio_venta,0)) AS '$fec[$j]',";
     }else{
         $text=$text." SUM(IF(DATE_FORMAT(fecha, '%Y-%m-%d')='$fec[$j]' and tipo_doc<=3 and ven_com=$q1 and activo=1 and detalle_factura.tienda$tienda,cantidad*precio_venta,0)) AS '$fec[$j]'";
     }
     }
