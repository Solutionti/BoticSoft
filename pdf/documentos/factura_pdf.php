...
//CREAR CLIENTE NUEVO O EDITAR CLIENTE SI EXISTE
    ...
    $pago_efectivo=$_GET['efectivo'];

if(empty($pago_efectivo) or $pago_efectivo==0){
  $pago_efectivo=0;
}
...
$numero_factura=$row77["tienda$suc"]+1;
$folio=$row77["folio$suc"];
...
while ($row=mysqli_fetch_array($sql))
	...
	$insert_detail=mysqli_query($con, "INSERT INTO detalle_factura VALUES (NULL,'$id_cliente','$id_vendedor','$numero_factura','$des','$id_producto','$cantidad','$precio_venta_r','$suc','$ddd','1','$date_added','$costo','$tipo','$d','$moneda1','$folio')");
...
$cuenta="";
$tienda="tienda".$c;
$documento=mysqli_query($con, "UPDATE documento SET $tienda=$tienda+1 WHERE id_documento=$tipo5");
...
$insert=mysqli_query($con,"INSERT INTO facturas VALUES (NULL,'$numero_factura','$date','0','$nro_doc','$id_cliente','0','$id_vendedor','$condiciones','$sumador_total','$deuda','$tipo','$suc','1','$ddd','1','$moneda1','$des','$obs','0','2018-11-11','$dias','$folio','$des','','0','$motivo','$tip','$pago_efectivo')");
...
$accion1=mysqli_query($con, "select * from facturas where numero_factura='".$numero_factura."' and folio='".$folio."' and estado_factura='".$tipo."' and tienda=$suc");
$row1=mysqli_fetch_array($accion1);
$id_factura=$row1["id_factura"];
if($online<>""){
   $online1=mysqli_query($con, "UPDATE factura_carrito SET activo=$id_factura WHERE codigo='$online'");
}
...
//DIRECCION DE PDF
//print"$valor34";
header("location:ver_ticket.php?id_factura=$id_factura&tipo=$tipo");
ob_end_flush();
?>
